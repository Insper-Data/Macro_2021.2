...
panel_dataset_lags <- panel_dataset %>% 
  ...
  mutate(
    ...
    lag_gdp_per_capta_usd_wb_1 = dplyr::lag(gdp_per_capta_usd_wb,1),
...
panel_dataset_lags_2 <- panel_dataset %>%
  ...
  mutate(
    ...
    lag_gdp_per_capta_usd_wb_2 = dplyr::lag(gdp_per_capta_usd_wb,2),
...
panel_dataset_lags_3 <- panel_dataset %>% 
  ...
  mutate(
    ...
    lag_gdp_per_capta_usd_wb_3 = dplyr::lag(gdp_per_capta_usd_wb,3),
...
panel_dataset_mean_lags <- panel_dataset %>%
  ...
  mutate(
    ...
    lag_gdp_per_capta_usd_wb_mean = (dplyr::lag(gdp_per_capta_usd_wb,1)+dplyr::lag(gdp_per_capta_usd_wb,2)+dplyr::lag(gdp_per_capta_usd_wb,3))/3,
...
## Excluding real_interest_rate and taxes: NA values
...
lag_0 <- subset(panel_dataset, select = c(GDP_growth, fx_volatility, GDP_per_cap_cur_USD, nominal_rate, account_balance, lending_borroeing_rate, unemployment, inflation_mean, debt_to_GDP, gdp_per_capta_usd_wb, ESGIp, Ep, Sp, Gp, foreign_debt_to_gdp, vix_EUA, vix_EUR, spreads))
lag_0 <- na.omit(lag_0)
lag_0 <- scale(lag_0)
...
lag_1 <- subset(panel_dataset_lags, select = c(lag_GDP_growth_1, lag_fx_volatility_1, lag_nominal_rate_1, lag_account_balance_1, lag_lending_borroeing_rate_1, lag_unemployment_1, lag_inflation_mean_1, lag_debt_to_GDP_1, lag_gdp_per_capta_usd_wb_1, lag_ESGIp_1, lag_Ep_1, lag_Sp_1, lag_Gp_1, lag_foreign_debt_to_gdp_1, lag_vix_EUA_1, lag_vix_EUR_1, spreads))
lag_1 <- na.omit(lag_1)
lag_1 <- scale(lag_1)
...
lag_3 <- subset(panel_dataset_lags_3, select = c(lag_GDP_growth_3, lag_fx_volatility_3, lag_nominal_rate_3, lag_account_balance_3, lag_lending_borroeing_rate_3, lag_unemployment_3, lag_inflation_mean_3, lag_debt_to_GDP_3, lag_gdp_per_capta_usd_wb_3, lag_ESGIp_3, lag_Ep_3, lag_Sp_3, lag_Gp_3, lag_foreign_debt_to_gdp_3, lag_vix_EUA_3, lag_vix_EUR_3, spreads))
lag_3 <- na.omit(lag_3)
lag_3 <- scale(lag_3)
...
mean <- subset(panel_dataset_mean_lags, select = c(lag_GDP_growth_mean, lag_fx_volatility_mean, lag_nominal_rate_mean, lag_account_balance_mean, lag_lending_borroeing_rate_mean, lag_unemployment_mean, lag_inflation_mean_mean, lag_debt_to_GDP_mean, lag_gdp_per_capta_usd_wb_mean, lag_ESGIp_mean, lag_Ep_mean, lag_Sp_mean, lag_Gp_mean, lag_foreign_debt_to_gdp_mean, lag_vix_EUA_mean, lag_vix_EUR_mean, spreads))
mean <- na.omit(mean)
mean <- scale(mean)
...
lambda.array <- seq(from = 0.01, to = 10, by = 0.01)
